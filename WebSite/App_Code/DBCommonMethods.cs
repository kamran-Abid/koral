...
" left join HR_Designation on Employee.Rank=HR_Designation.Designation_ID where Status = 'Active'";
    ...
    public static string[] GetCNICNos(string CNIC, string Type = "1", string Name = "", string Rank = "", string Status = "1")
    {
        ...
        string NICColumn = "Nic";
        string NameColumn = "FirstName";
        if (Type == "2")
        {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            {
                cmd.CommandText = "select " + NICColumn + " as Nic, " + NameColumn + " from " + recordTable + " where " +
                " " + NICColumn + " like'%" + CNIC + "%' ";
    ...
    public static string[] GetNames(string Name, string Type = "1", string CNIC = "", string Rank = "", string Status = "1")
    {
        string recordTable = "Employee";
        string NICColumn = "Nic";
        string NameColumn = "FirstName,LastName";
        Name = Name.ToLower();
        ...
        else if (Type == "4")
        {
            ...
            List<string> mctxVisistor = new List<string>();
            using (SqlConnection conn = new SqlConnection())
            {
                conn.ConnectionString = ConfigurationManager
                        .ConnectionStrings["vmsconnectionstring"].ConnectionString;
                using (SqlCommand cmd = new SqlCommand())
                {
                    cmd.CommandText = "select  Name from " + recordTable + " where " +
                    " LOWER(Name) like'%" + Name + "%' and Status = 'Active'";
                    //cmd.Parameters.AddWithValue("@SearchText", prefix);
                    cmd.Connection = conn;
                    conn.Open();
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            mctxVisistor.Add(string.Format("{0}", sdr["Name"]));
                        }
                    }
                    conn.Close();
                }
                return mctxVisistor.ToArray();
            }
    ...
    public static Dictionary<string, object> GetAllEmployee()
    {
        string Query = "select EID, FirstName + ' ' +LastName as Name from Employee";
        DBManager ObjDBManager = new DBManager();
        DataSet ds = ObjDBManager.SelectQuery(Query, "Employees", "vmsconnectionstring");
        DataTable dt = new DataTable();
        dt = ds.Tables["Employees"];
        return ToJson(dt);
    }

    public static Dictionary<string, object> GetAllDepartments()
    {
        string Query = "select Department_ID, Department_Name from mctx_Department";
        DBManager ObjDBManager = new DBManager();
        DataSet ds = ObjDBManager.SelectQuery(Query, "mctx_Department", "vmsconnectionstring");
        DataTable dt = new DataTable();
        dt = ds.Tables["mctx_Department"];
        return ToJson(dt);
    }

    public static Dictionary<string, object> GetAllGate()
    {
        string Query = "select Gate_ID, Gate_Name from mctx_Gates";
        DBManager ObjDBManager = new DBManager();
        DataSet ds = ObjDBManager.SelectQuery(Query, "mctx_Gates", "vmsconnectionstring");
        DataTable dt = new DataTable();
        dt = ds.Tables["mctx_Gates"];
        return ToJson(dt);
    }

    public static Dictionary<string, object> GetAllTerminal()
    {
        string Query = "select Terminal_ID, TerminalName from mctx_TerminalInfos";
        DBManager ObjDBManager = new DBManager();
        DataSet ds = ObjDBManager.SelectQuery(Query, "mctx_TerminalInfos", "vmsconnectionstring");
        DataTable dt = new DataTable();
        dt = ds.Tables["mctx_TerminalInfos"];
        return ToJson(dt);
    }
    ...
    public static Dictionary<string, object> getIDWiseReport(string ID, string Type = "1")
    {
        ...
        if (Type == "1")
        {
            Query = "SELECT EID,ServiceNo,CardNumber, Employee.Status,isnull(HR_Designation.designation,'') as Designation,Rnk.Rank_Name as Rank,isnull(convert(varchar(10), AllotmentDate,101),'') as AllotmentDate, " +  
     "FirstName,LastName,CurrAddr,PerAddr,Nic,convert(varchar(10), Dob,101) as Dob, PhoneOffice,PhoneHome,Mobile,  "+
     "isnull(convert(varchar(10), ReleaseDate,101),'') as ReleaseDate, "+
    ...
    public static Dictionary<string, object> getEmployeeFamily(string EID)
    {
        ...
        string Query = " SELECT ef.EID,ef.EFID,ef.CardNumber,ef.FirstName,ef.LastName, " +
                        ...
                        "left join mctx_Relation on ef.Relation=mctx_Relation.Relation_ID "+
                        "where ef.EID= " + EID;
    ...
    public static Dictionary<string, object> getEmployeeSerant(string EID)
    {
        ...
        string Query = "SELECT sr.SID,sr.EID,sr.CardNumber,sr.FirstName,sr.LastName,sr.FH,sr.CurrAddr,sr.PerAddr,sr.Caste,convert(varchar,sr.DOB,101) as DOB,sr.PlaceofWork, "+
                        ...
                        " left join mctx_Religion on sr.Religion=mctx_Religion.Religion_ID "+
                        " WHERE  sr.EID= " + EID;
    ...
    public static Dictionary<string, object> getEmployeeLogReport(string Employee_Id, string Card_Id, string Date_From, string Date_To, string Department_Id, string Designation_Id, string Gate_Id, string Terminal_Id)
    {
        ...
        string Query = @"select emp.FirstName + ' '+ emp.LastName  as name, emp.CardNumber, dept.Department_Name , desg.designation,
gat.Gate_Name , termI.TerminalName , vlog.Date from mctx_visitorlog  vlog " +
"inner join Employee emp on emp.EID = vlog.VID and User_Type= 'Employee'  " +
...
" inner join mctx_Gates gat on gat.Gate_ID=vlog.GATE_NUMBER " +
" inner join mctx_TerminalInfos termI on termI.Terminal_ID = vlog.TerminalID" +
" where ('" + Employee_Id + "'='' or emp.EID='" + Employee_Id + "') " +
" and ('" + Department_Id + "'='' or emp.Department='" + Department_Id + "') " +
" and ('" + Designation_Id + "'='' or emp.Designation='" + Designation_Id + "') " +
" and ('" + Card_Id + "'='' or emp.CardNumber='" + Card_Id + "') " +
" and ('" + Gate_Id + "'='' or vlog.GATE_NUMBER='" + Gate_Id + "') " +
" and ('" + Terminal_Id + "'='' or vlog.TerminalID='" + Terminal_Id + "') ";
//+
//" and ('"+Date_From+"' IS NULL OR  cast(vlog.DATE  as date)>= CAST('"+Date_From+"' AS date) AND cast(vlog.DATE  as date) < DATEADD(DD,1,CAST('"+Date_To+"' AS DATE)) )";

        DBManager ObjDBManager = new DBManager();
        DataSet ds = ObjDBManager.SelectQuery(Query, "EmployeeLogReport", "vmsconnectionstring");
        DataTable dt = new DataTable();
        dt = ds.Tables["EmployeeLogReport"];
        return ToJson(dt);
    ...
    public static Dictionary<string, object> getEmployeeNameWiseReport(string Name, string Type = "1", string CNIC = "", string Rank = "", string Status = "",string From="", string To="")
    {
        ...
        if (Type == "1")
        {
            Query = "SELECT top 100 EID,ServiceNo,CardNumber, Employee.Status,isnull(RNK.RANK_NAME,'') as Rank,isnull(convert(varchar(10), AllotmentDate,101),'') as AllotmentDate, " +
    " FirstName,LastName,CurrAddr,PerAddr,Nic,convert(varchar(10), Dob,101) as Dob, PhoneOffice,PhoneHome,Mobile,  " +
    " isnull(convert(varchar(10), ReleaseDate,101),'') as ReleaseDate, " +
    " host.[HOST_NAME] as Location,isnull(Department_Name,'') as Department,HR_Designation.designation as Designation,BarCode,NewPhoto as Photo, " +
    " _Status as [status] from Employee " +
    ...
    " where 1=1";
        ...
        else if (Type == "2")
        {
            ...
            Query = " SELECT top 100 nr.NRID,nr.CardNumber,nr.FirstName, nr.LastName,nr.RelName ,nr.CurrAddr,nr.PerAddr,nr.NIC,nr.PhoneHome, " +
                    " nr.PhoneOffice,nr.Mobile,relg.Religion_Name as Religion,nr.Sectt,cst.Caste_Name as Caste,nr.Education,nr.Witness1Name,nr.Witness1Addr,nr.Witness2Name,nr.Witness2Addr, " +
        ...
        else if (Type == "3")
        {
            ...
            //        "where (em.FirstName + ' ' + em.LastName) like '%" + Name + "%'";
            Query = " select top 100 v.FirstName, v.LastName, v.VID, isnull(v.CardNo,'') as CardNo, des.designation as Rank, v.NIC, v.Phone, v.Mobile, v.Addr, v.Profession, isnull(v.VisitPurpose,'') as VisitPurpose, " +
                   "  v.VisitDays, v.IssueDt, v.ValidUpto, v.ReferenceName, isnull(rel.Relation_Name,'') as Relation, v.Status, v.BarCode, v.ReferenceAdd as Reference, v.Photo " +
                   "  from Visitor v " +
        ...
        else if (Type == "4")
        {
            ...
            Query = "SELECT  top 100 [ID] ,[CNIC] as NIC,[VISITOR_NAME],[FATHER_NAME],[DOB],[ADDRESS],[DATE],[TIME_IN],[TIME_OUT] " +
             " ,[HOST],[VEHICLE_NUMBER],[TOKEN_NUMBER],[DEPARTMENT],[GATE_NUMBER],[NAME] as FirstName, '' as LastName,[PHONE_NUMBER] " +
             " ,[RESTRICTED],[PICTURE_ID],[mctx_visitor].[IsHandled],[Is_deleted],[Client_IP],[Relations],[Status] " +
             " ,[Visit_Purpose] as VisitPurpose,[NewPhoto],RFID AS CardNo " +
             " FROM [mctx_visitor] INNER JOIN dbo.mctx_VirdiUsers ON dbo.mctx_visitor.ID=dbo.mctx_VirdiUsers.User_ID " +
        ...
        if (!string.IsNullOrEmpty(CNIC))
            {
              //  Query += " and CNIC ='" + CNIC + "'";
                Query += " and ( CNIC ='" + CNIC + "' OR RFID = '" + CNIC + "' )";
            }
    ...
    */
    private static void EmployeeDetails(string[] empIds, string recordType)
    {
        ...
        ExcelRange range = xlWorkSheetToExport.Cells[iRowCnt,1,iRowCnt,8];
        ...
        xlWorkSheetToExport.Cells[iRowCnt, 4].Value = "Designation";
        xlWorkSheetToExport.Cells[iRowCnt, 5].Value = "NIC";
        //xlWorkSheetToExport.Cells[iRowCnt, 5].Value = "Date of Birth";
        //xlWorkSheetToExport.Cells[iRowCnt, 6].Value = "Location";
        xlWorkSheetToExport.Cells[iRowCnt, 6].Value = "Department";
      ...
      xlWorkSheetToExport.Cells[iRowCnt, 7].Value = "Status";
        ...
        for (j = 0; j < empIds.Count(); j++)
        {
            ...
            for (int i = 0; i < employeeFamily.Count; i++)
            {
                ...
                xlWorkSheetToExport.Cells[iRowCnt, 4].Value = employeeFamily[i]["Designation"];
                xlWorkSheetToExport.Cells[iRowCnt, 5].Value = employeeFamily[i]["Nic"];
                ...
                xlWorkSheetToExport.Cells[iRowCnt, 6].Value = employeeFamily[i]["Department"];
                ...
                xlWorkSheetToExport.Cells[iRowCnt, 8].Value = employeeFamily[i]["Status"];
    ...
    private static void VMSVisitorDetails(string[] empIds, string recordType)
    {
        ...
        // SHOW THE HEADER.
        xlWorkSheetToExport.Cells[iRowCnt, 1].Value = headerName + " Search Details";
        ExcelRange range = xlWorkSheetToExport.Cells[iRowCnt, 1, iRowCnt, 9];
        range.Style.Font.Bold = true;
        range.Style.Fill.PatternType = ExcelFillStyle.Solid;
        range.Style.Fill.BackgroundColor.SetColor(Color.DarkBlue);
        range.Style.Font.Color.SetColor(Color.White);
        ...
        //xlWorkSheetToExport.Cells[iRowCnt, 5].Value = "Mobile";
        //xlWorkSheetToExport.Cells[iRowCnt, 6].Value = "Address";
        xlWorkSheetToExport.Cells[iRowCnt, 5].Value = "Visited Person";
        xlWorkSheetToExport.Cells[iRowCnt, 6].Value = "Visit Purpose";
        //xlWorkSheetToExport.Cells[iRowCnt, 9].Value = "Visit Days";
        //xlWorkSheetToExport.Cells[iRowCnt, 10].Value = "Issue Date";
        //xlWorkSheetToExport.Cells[iRowCnt, 11].Value = "Valid Upto";
        //xlWorkSheetToExport.Cells[iRowCnt, 12].Value = "Reference Name";
        xlWorkSheetToExport.Cells[iRowCnt, 7].Value = "Address";
        //xlWorkSheetToExport.Cells[iRowCnt, 14].Value = "Rank";
        //xlWorkSheetToExport.Cells[iRowCnt, 15].Value = "Reference";
        xlWorkSheetToExport.Cells[iRowCnt, 8].Value = "Status";
        xlWorkSheetToExport.Cells[iRowCnt, 9].Value = "Photo";

        range = xlWorkSheetToExport.Cells[iRowCnt, 1, iRowCnt, 9];
        range.Style.Font.Bold = true;
        range.Style.Fill.PatternType = ExcelFillStyle.Solid;
        range.Style.Fill.BackgroundColor.SetColor(Color.LightBlue);
        range.Style.Font.Color.SetColor(Color.White);
        ...
        for (j = 0; j < empIds.Count(); j++)
        {
            ...
            for (int i = 0; i < employeeFamily.Count; i++)
            {
                ...
                //xlWorkSheetToExport.Cells[iRowCnt, 5].Value = employeeFamily[i]["Mobile"];
                //xlWorkSheetToExport.Cells[iRowCnt, 6].Value = employeeFamily[i]["Addr"];
                xlWorkSheetToExport.Cells[iRowCnt, 5].Value = employeeFamily[i]["HOST"];
                xlWorkSheetToExport.Cells[iRowCnt, 6].Value = employeeFamily[i]["VisitPurpose"];
                //xlWorkSheetToExport.Cells[iRowCnt, 9].Value = employeeFamily[i]["VisitDays"];

                //xlWorkSheetToExport.Cells[iRowCnt, 10].Value = employeeFamily[i]["IssueDt"];
                //xlWorkSheetToExport.Cells[iRowCnt, 11].Value = employeeFamily[i]["ValidUpto"];
                //xlWorkSheetToExport.Cells[iRowCnt, 12].Value = employeeFamily[i]["ReferenceName"];
                xlWorkSheetToExport.Cells[iRowCnt, 7].Value = employeeFamily[i]["ADDRESS"];
                //xlWorkSheetToExport.Cells[iRowCnt, 14].Value = employeeFamily[i]["Rank"];
                //xlWorkSheetToExport.Cells[iRowCnt, 15].Value = employeeFamily[i]["Reference"];
                //xlWorkSheetToExport.Cells[iRowCnt, 15] = employeeFamily[i]["Photo"]; 
                xlWorkSheetToExport.Cells[iRowCnt, 8].Value = employeeFamily[i]["Status"];
    ...
    //public int SP_tblone(string name1)
    //{
    //    //string Query = "select Gate_ID, Gate_Name from mctx_Gates";
    //    DBManager ObjDBManager = new DBManager();
