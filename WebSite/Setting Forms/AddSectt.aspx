<%@ Page Title="" Language="C#" MasterPageFile="~/PACS.master" AutoEventWireup="true" CodeFile="AddSectt.aspx.cs" Inherits="AddSectt" %>
              ...
              if ($("#tblSecttDetails tr").length - 1 > 1) {
                  for (i = 0; i < $("#tblSecttDetails tr").length - 1; i++) {
                      ...
                      if ($("#txtSecttName" + i).html().toUpperCase().trim() == $("input[id$='txtSecttName']").val().toUpperCase().trim()) {
                          alert("Sectt " + $("input[id$='txtSecttName']").val() + " is already added!");
