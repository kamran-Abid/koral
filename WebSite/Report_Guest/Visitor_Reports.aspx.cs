...
public partial class Reposts_GuestReports_Visitor_Reports : System.Web.UI.Page
{
    ...
    protected void btnsearch_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            ds = db_Manager.ExecuteDataSet("Guest_ReportS", "vmsconnectionstring");
            ReportViewer1.Visible = true;
            ReportDataSource rds = new ReportDataSource("VisitorDataSet", ds.Tables[0]);
    ...
    #region----Get Visitor Name AutoComplete
    [WebMethod]

    public static List<string> GetVisitorNames(string term)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("GetGuestName_SP", con);
            ...
            while (rdr.Read())
            {
                listCountryName.Add(rdr["Name"].ToString());
            }
            con.Close();
    ...
    #region AutoComplet Visitor CNIC
    [WebMethod]
    public static List<string> GetVisitorCNIC(string term)
    {
        List<string> listCountryName = new List<string>();
        string CS = ConfigurationManager.ConnectionStrings["vmsconnectionstring"].ConnectionString;
        using (SqlConnection con = new SqlConnection(CS))
